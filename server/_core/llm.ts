...
const resolveLlmProvider = (): LlmProvider => {
  ...
  throw new Error("AI is not configured. Set OPENAI_API_KEY for a self-hosted deployment.");
};
...
export async function invokeLLM(params: InvokeParams): Promise<InvokeResult> {
  ...
  if (model || provider.defaultModel) {
    payload.model = model || provider.defaultModel;
  }
  ...
  const response = await fetchWithBackoff(provider.apiUrl, {
    method: "POST",
    headers: {
...
export async function listLLMModels(): Promise<ModelsResponse> {
  const provider = resolveLlmProvider();
  const url = provider.apiUrl.replace(/\/v1\/chat\/completions$/, "/v1/models");
