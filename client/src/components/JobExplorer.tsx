    ...
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          <span className="rounded-full bg-[#e7f8f5] px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-[#177a72]">{job.jobType === "walk_in" ? "Walk-in" : "Job posting"}</span>
          <span className="text-[11px] text-[#75919e]">#{job.id}</span>
        </div>
        <h3 className="mt-2 text-lg font-bold tracking-[-0.04em] text-[#1c3e54]">{job.role}</h3>
        <p className="mt-1 text-sm font-semibold text-[#52778a]">{job.company}</p>
      </div>
      {job.url && <a href={job.url} target="_blank" rel="noreferrer" className="shrink-0 text-xs font-bold text-[#216fc9] hover:text-[#12549b]">Open listing ↗</a>}
    </div>
    <p className="mt-4 text-sm leading-5 text-[#5e7c8b]">{job.description}</p>
    <div className="mt-4 flex flex-wrap gap-1.5">{job.skills.map(skill => <span key={skill} className="rounded-md bg-[#f0faf9] px-2 py-1 text-[10px] font-semibold text-[#237773]">{skill}</span>)}</div>
...
export default function JobExplorer() {
  ...
  return <section className="mb-9 rounded-[1.7rem] border border-white/90 bg-white/75 p-6 shadow-[0_18px_44px_rgba(18,79,110,0.08)] backdrop-blur-xl sm:p-7">
    ...
    <div className="mt-6 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
      ...
      <label><span className="mono-label mb-1.5 block text-[9px] text-[#577d8e]">Open date to</span><input type="date" value={draftFilters.openDateTo} onChange={event => setField("openDateTo", event.target.value)} className="h-10 w-full rounded-lg border border-[#d6e9ed] bg-white px-3 text-sm outline-none focus:border-[#53c9c1]" /></label>
      <label><span className="mono-label mb-1.5 block text-[9px] text-[#577d8e]">Closed date from</span><input type="date" value={draftFilters.closedDateFrom} onChange={event => setField("closedDateFrom", event.target.value)} className="h-10 w-full rounded-lg border border-[#d6e9ed] bg-white px-3 text-sm outline-none focus:border-[#53c9c1]" /></label>
      <label><span className="mono-label mb-1.5 block text-[9px] text-[#577d8e]">Closed date to</span><input type="date" value={draftFilters.closedDateTo} onChange={event => setField("closedDateTo", event.target.value)} className="h-10 w-full rounded-lg border border-[#d6e9ed] bg-white px-3 text-sm outline-none focus:border-[#53c9c1]" /></label>
      <label><span className="mono-label mb-1.5 block text-[9px] text-[#577d8e]">Sort by</span><select value={draftFilters.sortBy} onChange={event => setField("sortBy", event.target.value as JobFilters["sortBy"])} className="h-10 w-full rounded-lg border border-[#d6e9ed] bg-white px-3 text-sm outline-none focus:border-[#53c9c1]"><option value="latest">Newest first</option><option value="closing">Closing soon</option><option value="company">Company name</option></select></label>
    </div>
